...
uncompressed, deterministic, bounded, and never extracted using ZipFile.extract(). The limits
mirror challenges.json; a test keeps them equal. Entries inside the ZIP live under the track's
submission root so an unpacked archive lands where a submissions checkout keeps the root.
"""
...
DECLARED = {"sigma.txt", "hverify.txt", "bound.txt", "NOTES.md", "README.md"}
...
def _name(name: str) -> bool:
    return bool(LEAN_FILE.fullmatch(name)) or name in DECLARED
...
def save_source(store: Path, submission_id: str, root: Path, *, source_repo: str, commit: str,
    ...
    """Snapshot a staged flat root and commit its binding before untrusted compilation starts."""
...
def restore_source(store: Path, metadata: dict, destination: Path, *, commit: str, track: str,
    ...
    The files are written flat into `destination`, the staged root this verifier hands to
    verify_submission.py; the submission root path is only a name inside the ZIP.
    """
