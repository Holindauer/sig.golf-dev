...
class SubmissionsRepositoryTests(unittest.TestCase):
    """Runs against a fixture core built from this checkout's template and challenges.json, so it
    needs neither a clean working tree nor the real contract pin."""
    ...
    def setUp(self):
        ...
        stray = self.root / 'formal/Submissions/Full'
        stray.mkdir(parents=True)
        (stray / 'Solution.lean').write_text('def fixture := 1\n')
        (stray / 'sigma.txt').write_text('7\n')
    ...
    def test_export_is_a_separate_repo_with_an_exact_core_pin(self):
        ...
        self.assertEqual(result['tracks'], ['full'])
        ...
        self.assertEqual(names, {'.contract', '.github/PULL_REQUEST_TEMPLATE.md', '.gitignore', '.gitmodules',
                                 'AGENTS.md', 'LICENSE', 'README.md', 'records.json'})
        self.assertEqual((destination / 'LICENSE').read_text(), 'Fixture license\n')
        self.assertEqual(json.loads((destination / 'records.json').read_text()), {'version': 1, 'records': {}})
        readme = (destination / 'README.md').read_text()
        self.assertIn(commit, readme)
        self.assertIn(result['contract_id'], readme)
        for name in ('README.md', 'AGENTS.md', '.github/PULL_REQUEST_TEMPLATE.md'):
            self.assertNotIn('{{CONTRACT_', (destination / name).read_text(), name)
