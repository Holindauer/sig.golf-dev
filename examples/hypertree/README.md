...
Signature and witness size: 119,632 bytes each. Key generation is proved to succeed for every seed and fixed oracle in 81,342 cycles, with 739 hash calls and 761 compressions. Signing is proved to use exactly 117,508 hash calls and 121,008 compressions, with at most 16,800,271 cycles.
...
- Exact equality between the actual organizer security experiment and the reference security experiment, preserving private coins, signing history, cache handling, lifetime limits, both forgery forms, and total hash-call counts.
- Concrete random-oracle query and cache-replacement lemmas, seed-guessing bounds, exact private/public oracle simulations of reference key generation and signing, a seed-erasure game hop, monadic verification, hash-call cutoff accounting, randomized-index bounds, full-path forgery extraction, a concrete reference security experiment, independent uniform labels for the complete addressed hash graph, equivalence between hidden graph precomputation and the observable lazy random oracle, and exact signing-output dependence on revealed graph points. Passive adaptive contact bounds and signing-index collision bounds are checked separately, with exact expected query counters. Their composition into the full reference-game forgery bound remains unfinished.
- Exact graph-oracle key-generation and signing simulations: key generation leaves the residual oracle unchanged, and signing performs exactly one residual H5 query. Canonical signing disclosures and deterministic forgery extraction are transported to arbitrary independent private tables.
...
The actual organizer statements for admission, universal termination, completeness, exponential compression budgets, and verification cycles are proved. Security remains open: the adaptive contact and randomized-index bounds still need to be combined into the full forgery bound. The charged-budget simulation between the bytecode and reference security experiments is proved. `certificate_of_secure` explicitly requires that missing security theorem; no complete `SigGolf.Certificate` is claimed.
