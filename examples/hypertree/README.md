...
The four RISC-V images have a complete Lean certificate: `SigGolfCandidate.Hypertree.Candidate.certificate : SigGolf.Certificate submission 3805795`, in [Certificate.lean](../../SigGolfCandidate/Hypertree/Certificate.lean).

| Parameter | Value |
| --- | ---: |
| Signature bytes S | 119,632 |
| Witness bytes W | 119,632 |
| Maximum honest verification cycles C | 3,805,795 |
| Score S × C | 455,294,867,440 |
...
| Verification | 3,805,795 | ≤ 51,841 | ≤ 53,602 |
...
The bottom layer is bounded separately: 116 cycles for its leaf, 236 for its tree, and 288 for the full layer including dispatch. The complete bound is `145 + 288 + 159 * 23933 + 15 = 3805795` cycles.

The verifier unrolls the two 16-byte copies inside each chain hash. Each replacement preserves all registers, memory, and the following program counter while saving seven cycles. Together with the shared-base header, the complete chain iteration costs 66 cycles instead of 103. `FastCopy16.lean` proves the replacement and connects it to the actual verifier image.
