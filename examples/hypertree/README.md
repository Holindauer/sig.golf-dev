...
The four RISC-V images have a complete Lean certificate: `SigGolfCandidate.Hypertree.Candidate.certificate : SigGolf.Certificate submission 4932151`, in [Certificate.lean](../../SigGolfCandidate/Hypertree/Certificate.lean).

| Parameter | Value |
| --- | ---: |
| Signature bytes S | 119,632 |
| Witness bytes W | 119,632 |
| Maximum honest verification cycles C | 4,932,151 |
| Score S × C | 590,043,088,432 |
...
| Verification | 4,932,151 | ≤ 51,841 | ≤ 53,602 |
...
The bottom layer is bounded separately: 116 cycles for its leaf, 236 for its tree, and 288 for the full layer including dispatch. The complete bound is `145 + 288 + 159 * 31017 + 15 = 4932151` cycles.

The verifier unrolls the two 16-byte copies inside each chain hash. Each replacement preserves all registers, memory, and the following program counter while saving seven cycles. The complete chain iteration costs 89 cycles instead of 103. `FastCopy16.lean` proves the replacement and connects it to the actual verifier image.
