...
theorem sign_selected_leaf_call (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    ...
    (data : LeafContext s seed level tree side) (settings : LeafSignatureSettings s pointer message) :
  ...
  obtain ⟨final, steps, cycles, body, stepsBound, cyclesBound, finalPC, finalSP, root, signature, frame⟩ :=
    sign_selected_leaf_body hash (leafReady s) seed pointer level tree side message readyPC (leafReady_sp s sp)
      upper valid (leafReady_data s seed level tree side sp data) (leafReady_settings s pointer message sp settings)
...
theorem sign_unselected_leaf_call (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    (side : Bool) (pc : s.pc = 0x154c) (sp : s.getReg .x2 = 0xfffff0)
    (nonzero : BitVec.ofNat 64 level ≠ 0) (valid : CapturePointerValid pointer)
    (data : LeafContext s seed level tree side) (ptr : s.getMem 0x80448 = BitVec.ofNat 64 pointer)
  ...
  obtain ⟨final, steps, cycles, body, stepsBound, cyclesBound, finalPC, finalSP, root, frame⟩ :=
    sign_unselected_leaf_body hash (leafReady s) seed pointer level tree side readyPC (leafReady_sp s sp)
      upper valid (leafReady_data s seed level tree side sp data) readyPtr readyUnselected
...
theorem sign_bottom_leaf_call (hash : Hash) (s : MachineState) (seed : Seed) (pointer tree : Nat)
    (side : Bool) (pc : s.pc = 0x154c) (sp : s.getReg .x2 = 0xfffff0)
    (valid : CapturePointerValid pointer) (ptr : s.getMem 0x80448 = BitVec.ofNat 64 pointer)
    (data : LeafContext s seed 0 tree side) :
  ...
  obtain ⟨final, steps, cycles, body, stepsBound, cyclesBound, finalPC, finalSP, root, lowFrame, frame⟩ :=
    sign_bottom_leaf_body hash (leafReady s) seed pointer tree side readyPC (leafReady_sp s sp) valid readyPtr
      (leafReady_data s seed 0 tree side sp data) (leafReady_step s)
