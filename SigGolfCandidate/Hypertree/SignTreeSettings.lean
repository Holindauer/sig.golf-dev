...
theorem TreeSettings.selected (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    (message : Reference.Digest) (selected : Bool) (settings : TreeSettings s pointer message selected)
    (data : LeafContext s seed level tree selected) : LeafSignatureSettings s pointer message := by
  intro chain
  exact ⟨settings.pointerEq, settings.enabled, data.leafEq.trans settings.selectorEq.symm, settings.digits chain⟩

theorem TreeSettings.unselected (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    (message : Reference.Digest) (side selected : Bool) (settings : TreeSettings s pointer message selected)
    (data : LeafContext s seed level tree side) (different : side ≠ selected) :
...
theorem sign_upper_leaf_call (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    ...
    (data : LeafContext s seed level tree side) (settings : TreeSettings s pointer message selected) :
    ∃ final instructions cycles, Trace hash sign s instructions cycles 369 380 final ∧
      ...
      (side = selected → SignatureBefore final hash seed pointer level tree side message 46) ∧
  ...
  · subst side
    obtain ⟨final, steps, cycles, trace, hs, hc, fpc, fsp, root, signature, frame⟩ :=
      sign_selected_leaf_call hash s seed pointer level tree selected message pc sp nonzero valid data
        (settings.selected s seed pointer level tree message selected data)
  ...
  · obtain ⟨final, steps, cycles, trace, hs, hc, fpc, fsp, root, frame⟩ :=
      sign_unselected_leaf_call hash s seed pointer level tree side pc sp nonzero valid data settings.pointerEq
        (settings.unselected s seed pointer level tree message side selected data same)
