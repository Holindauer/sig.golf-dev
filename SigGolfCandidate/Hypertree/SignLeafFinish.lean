...
theorem EndpointsBefore.words (s : MachineState) (hash : Hash) (seed : Seed) (level tree : Nat) (side : Bool)
    (endpoints : EndpointsBefore s hash seed level tree side 46) :
...
theorem leaf_finish (hash : Hash) (s : MachineState) (seed : Seed) (level tree : Nat) (side : Bool)
    (pc : s.pc = 0x18c8) (sp : s.getReg .x2 = 0xffffe0) (upper : level ≠ 0)
    (data : LeafData s seed level tree side 46)
    (endpoints : EndpointsBefore s hash seed level tree side 46) :
  ...
  obtain ⟨final, trace, finalPC, finalSP, root, frame⟩ := KeygenLeaf.compute_return sign hash 0x18c8
    sign_leaf_compress_code sign_leaf_return_code s pc level tree side (Reference.endpoint hash seed level tree side)
    data.levelEq data.leafEq data.indexEq (endpoints.words s hash seed level tree side)
    (by rw [sp]; decide)
...
theorem sign_selected_leaf_body (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    (side : Bool) (message : Reference.Digest) (pc : s.pc = 0x1584) (sp : s.getReg .x2 = 0xffffe0)
    (upper : level ≠ 0) (valid : CapturePointerValid pointer)
    (data : LeafData s seed level tree side 0) (settings : LeafSignatureSettings s pointer message) :
  ...
  obtain ⟨ready, steps, cycles, pre, stepsBound, cyclesBound, readyPC, readyData, endpoints, signature,
    readyRA, readySP, readyFrame⟩ := sign_selected_leaf_chains hash s seed pointer level tree side message pc upper valid data settings
  have rsp : ready.getReg .x2 = 0xffffe0 := readySP.trans sp
  obtain ⟨final, post, finalPC, finalSP, root, finalFrame⟩ := leaf_finish hash ready seed level tree side readyPC rsp upper readyData endpoints
...
theorem sign_unselected_leaf_body (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    (side : Bool) (pc : s.pc = 0x1584) (sp : s.getReg .x2 = 0xffffe0)
    (upper : level ≠ 0) (valid : CapturePointerValid pointer)
    (data : LeafData s seed level tree side 0) (ptr : s.getMem 0x80448 = BitVec.ofNat 64 pointer)
  ...
  obtain ⟨ready, steps, cycles, pre, stepsBound, cyclesBound, readyPC, readyData, endpoints,
    readyRA, readySP, readyFrame⟩ := sign_unselected_leaf_chains hash s seed pointer level tree side pc valid data ptr unselected
  have rsp : ready.getReg .x2 = 0xffffe0 := readySP.trans sp
  obtain ⟨final, post, finalPC, finalSP, root, finalFrame⟩ := leaf_finish hash ready seed level tree side readyPC rsp upper readyData endpoints
