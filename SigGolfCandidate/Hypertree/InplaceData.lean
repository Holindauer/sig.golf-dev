...
theorem recurrent (image : Image) (hash : Hash) (p : Word)
    (prepareCode : InplacePrepare.Code image p) (coreCode : InplaceCore.Code image (p+96))
    ...
    (ready : CachedPrepare.Ready s) (data : Buffered s level tree side chain step value) :
    ∃ final, Trace hash image s 15 22 1 1 final ∧ final.pc = p-12 ∧
  ...
  obtain ⟨final,core,finalPC,valueEq,finalReady,finalBase,finalConstant,stepEq,ra,sp,frame⟩ :=
  ...
  refine ⟨final,(InplacePrepare.block image p prepareCode s pc base).trace.trans core,?_,?_,finalReady,
    finalBase,finalConstant,ra.trans (InplacePrepare.preserved s).1,sp.trans (InplacePrepare.preserved s).2.1,?_⟩
