...
theorem sign_unselected_leaf_loop (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree start remaining : Nat)
    (side : Bool)
    (pc : s.pc = if start = 46 then 0x18c8 else 0x1584) (length : start + remaining = 46)
    (valid : CapturePointerValid pointer)
    (data : LeafData s seed level tree side start)
  ...
  | succ remaining ih =>
    ...
    obtain ⟨next, steps, cycles, pre, stepsBound, cyclesBound, nextPC, nextData, endpoint,
      nextRA, nextSP, nextFrame⟩ := sign_unselected_iteration hash s seed pointer level tree side chain
    ...
    have nextUnselected : next.getMem 0x80428 ≠ next.getMem 0x80420 := by
      ...
      exact unselected
    have nextEndpoints : EndpointsBefore next hash seed level tree side (start+1) := by
      intro other less i
...
theorem sign_unselected_leaf_chains (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
  ...
  sign_unselected_leaf_loop hash s seed pointer level tree 0 46 side pc (by decide) valid data ptr unselected
