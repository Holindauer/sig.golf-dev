...
def Code (image : Image) (p : Word) : Prop :=
  ...
  instructionAt image (p+12) = some (.base (.ADDI .x10 .x0 2)) ∧
  CopyCode image (p+16)
...
theorem setup_block (image : Image) (p : Word) (code : Code image p)
  ...
  · rfl
  apply OrdinarySteps.step s3 (setup s) _ (.base (.ADDI .x10 .x0 2)) 0
...
theorem setup_regs (s : MachineState) :
    (setup s).getReg .x6 = 0x20 ∧ (setup s).getReg .x7 = 0x80020 ∧ (setup s).getReg .x10 = 2 := by
...
theorem copy (image : Image) (p : Word) (code : Code image p)
  ...
  have inv : CopyInvariant (p+16) 0x20 0x80020 2 2 (setup s) := by
    have regs := setup_regs s
    simp [CopyInvariant,setup_pc,pc,regs.1,regs.2.1,regs.2.2]
  obtain ⟨final,trace,done,content,frame,ra,sp⟩ :=
    copy_all_frame image (p+16) code.2.2.2.2 0x20 0x80020 2 (setup s) inv
      (by decide) (by decide) (by decide) (by decide) (by decide)
  refine ⟨final,ordinary_trans image _ _ _ 4 12 (setup_block image p code s pc) trace,?_,?_,
