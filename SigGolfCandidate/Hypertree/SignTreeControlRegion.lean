...
theorem right_region (hash : Hash) (s : MachineState) (pc : s.pc = 0x13e4) :
    RegionTrace (fun st => 0x1004 ≤ st.pc.toNat ∧ st.pc.toNat < 0x1c70)
      hash sign s 5 5 0 0 (state s 1 344) := by
  let p : Word := 0x13e4
  let side : BitVec 12 := 1
  let jump : BitVec 21 := 344
  have code : Code sign p side jump := by decide
  let s1 := execInstrBr s (.ADDI .x6 .x0 side)
  let s2 := execInstrBr s1 (.LUI .x28 128)
  let s3 := execInstrBr s2 (.ADDI .x28 .x28 0x428)
...
/-- info: 'SigGolfCandidate.Hypertree.KeygenTreeControl.right_region' depends on axioms: [propext, Quot.sound] -/
#guard_msgs in
#print axioms right_region
