...
abbrev Result := SecuritySeedMonitor.Outcome JointResult

/-- All four bad events and all three charged query classes share one execution.
The uniform seed is independent of the public simulation and is used only here. -/
noncomputable def experiment (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat) : ProbComp Result :=
  SecuritySeedMonitor.experiment (joint publicCache adversary rounds budget) (fun result => result.2.value.history.seedInputs)

def history (result : Result) : History := result.value.2.value.history

def SeedBad (result : Result) : Prop := result.bad = true
...
def Bad (result : Result) : Prop := SeedBad result ∨ GraphBad result ∨ NonceBad result ∨ IndexBad result
...
theorem supported (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat)
  ...
  simp only [experiment, SecuritySeedMonitor.experiment, mem_support_bind_iff,
    support_pure, Set.mem_singleton_iff] at member
  obtain ⟨value, member, seed, _, same⟩ := member
  cases same
  exact member
...
theorem seed_bound (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat) :
    Pr[SeedBad | experiment publicCache adversary rounds budget] ≤
      expectedValue (experiment publicCache adversary rounds budget) (fun result => ((history result).counts.seed : ENNReal)/2^128) := by
  have bound := SecuritySeedMonitor.prob_bad_le_expected (joint publicCache adversary rounds budget)
    (fun result => result.2.value.history.seedInputs)
  change Pr[SeedBad | experiment publicCache adversary rounds budget] ≤
  ...
  have calls : result.calls ≤ (history result).seedInputs.length := by
    have original := member
    simp only [experiment, SecuritySeedMonitor.experiment, mem_support_bind_iff,
      support_pure, Set.mem_singleton_iff] at original
    obtain ⟨value, _, seed, _, same⟩ := original
    cases same
    exact List.length_filter_le _ _
  have small : result.calls ≤ (history result).counts.seed := calls.trans_eq counted.2.2.2
...
theorem graph_bound (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat) :
  ...
  rw [SecuritySeedMonitor.value_event (joint publicCache adversary rounds budget)
    (fun result => result.2.value.history.seedInputs) (fun result => result.2.bad = true),
    SecuritySeedMonitor.expected_value (joint publicCache adversary rounds budget)
    (fun result => result.2.value.history.seedInputs) (fun result => 2*(result.2.value.history.counts.graph : ENNReal)/2^128)]
...
theorem nonce_bound (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat) :
  ...
  simp only [experiment, history]
  rw [SecuritySeedMonitor.value_event (joint publicCache adversary rounds budget)
    (fun result => result.2.value.history.seedInputs) (fun result => hits result.1 result.2.value.history = true),
    SecuritySeedMonitor.expected_value (joint publicCache adversary rounds budget)
    (fun result => result.2.value.history.seedInputs) (fun result => (result.2.value.history.counts.index : ENNReal)/2^256)]
...
theorem index_bound (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat) :
  ...
  simp only [experiment, history]
  rw [SecuritySeedMonitor.value_event (joint publicCache adversary rounds budget)
    (fun result => result.2.value.history.seedInputs) (fun result => SecurityIndexTrace.Conflict result.2.value.history.indexTrace),
    SecuritySeedMonitor.expected_value (joint publicCache adversary rounds budget)
    (fun result => result.2.value.history.seedInputs) (fun result => (result.2.value.history.counts.index : ENNReal)/2^136)]
...
theorem bad_le (publicCache : Cache) (adversary : Adversary submission.sizes) (rounds budget : Nat) :
  ...
  SecuritySharedBudget.prob_union_le (experiment publicCache adversary rounds budget)
    (fun result => (history result).counts) budget SeedBad GraphBad NonceBad IndexBad
    (count_le publicCache adversary rounds budget) (seed_bound publicCache adversary rounds budget)
