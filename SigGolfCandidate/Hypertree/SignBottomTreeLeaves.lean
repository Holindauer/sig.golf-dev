...
theorem sign_bottom_tree_leaves (hash : Hash) (s : MachineState) (seed : Seed) (pointer tree : Nat)
    (selected : Bool) (pc : s.pc = 0x13c8) (sp : s.getReg .x2 = 0x1000000)
    (valid : CapturePointerValid pointer)
    (data : TreeContext s seed 0 tree) (settings : BottomTreeSettings s pointer selected) :
    ∃ final instructions cycles, Trace hash sign s instructions cycles 4 4 final ∧
      ...
      final.getMem 0xfffff0 = s.getReg .x1 ∧ TreeContext final seed 0 tree ∧
      ...
      CapturedValue final pointer 0 (Reference.secret hash seed 0 tree selected 0) ∧
  ...
  have readyContext := treeLeft_context s seed 0 tree sp data
  ...
  obtain ⟨left, leftSteps, leftCycles, leftTrace, leftStepsBound, leftCyclesBound, leftPC, leftSP, leftRoot,
    leftLow, leftFrame⟩ := sign_bottom_leaf_call hash ready seed pointer tree false
  ...
  have readyTree : TreeContext ready seed 0 tree := ⟨readyContext.levelEq, readyContext.indexEq, readyContext.seedEq⟩
  have leftContext := treeContext_after_bottom_leaf ready left seed pointer tree false valid readyTree leftFrame
  ...
  have rightContext := treeControl_context left seed 0 tree true 344 leftContext
  ...
  obtain ⟨right, rightSteps, rightCycles, rightTrace, rightStepsBound, rightCyclesBound, rightPC, rightSP, rightRoot,
    rightLow, rightFrame⟩ := sign_bottom_leaf_call hash rightReady seed pointer tree true
  ...
  have rightTree : TreeContext rightReady seed 0 tree := ⟨rightContext.levelEq, rightContext.indexEq, rightContext.seedEq⟩
  have finalContext := treeContext_after_bottom_leaf rightReady right seed pointer tree true valid rightTree rightFrame
  ...
  · intro side i
    ...
    · exact rightRoot i
  · cases selected
    · have old := bottom_selected_value ready left hash seed pointer tree false valid readyContext readySettings leftLow
      intro i
      have low := signature_word_low pointer valid 0 i
      rw [bottom_unselected_low rightReady right hash seed pointer tree true false rightContext rightSettings
        (by decide) rightLow _ low, KeygenTreeControl.mem, if_neg (low_ne_high _ _ low (by decide))]
      exact old i
    · exact bottom_selected_value rightReady right hash seed pointer tree true valid rightContext rightSettings rightLow
