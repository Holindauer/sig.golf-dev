...
theorem execute_framed (hash : Hash) (s : MachineState) (pc : s.pc=0x1048)
  ...
  have leftCtx := context_after_leaf ready left level tree false seed rctx leftFrame
  ...
  have rrctx := control_context left false true 344 level tree seed leftCtx
  obtain ⟨right,rightTrace,rightPC,rightSP,rightWords,rightFrame⟩ :=
    KeygenLeafCall.execute hash rightReady rrpc rrsp level tree true seed nonzero rrctx
  have rightRet : right.pc=0x1078 := by rw [rightPC,rrra]; decide
  have rightStack : right.getReg .x2=0xfffff0 := rightSP.trans rrsp
  have rightCtx := context_after_leaf rightReady right level tree true seed rrctx rightFrame
  ...
  have leftPair : ∀ i : Fin 2, nodeReady.getMem (KeygenSavePublic.wordAddress false i.val)=
      (Reference.leafRoot hash seed level tree false).extractLsb' (64*i.val) 64 := by
    intro i
  ...
  have rightPair : ∀ i : Fin 2, nodeReady.getMem (KeygenSavePublic.wordAddress true i.val)=
      (Reference.leafRoot hash seed level tree true).extractLsb' (64*i.val) 64 := by
