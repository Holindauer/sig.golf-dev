...
structure Context (level tree : Nat) (side : Bool) (seed : Seed) (s : MachineState) : Prop where
  ...
  seedWords : ∀ i : Fin 2, s.getMem (Signing.wordAddress 0x20 i.val) = seed.extractLsb' (64*i.val) 64
...
theorem prepare (hash : Hash) (s : MachineState) (pc : s.pc=0x1204)
    ...
    ∃ final, Trace hash keygen s 81 88 1 1 final ∧ final.pc=0x1318 ∧
      KeygenChainLoop.Invariant level tree 0 side chain (Reference.secret hash seed level tree side chain) final ∧
      final.getReg .x1=s.getReg .x1 ∧ final.getReg .x2=s.getReg .x2 ∧
      (∀ a, KeygenChainLoop.Outside a → final.getMem a=s.getMem a) := by
  obtain ⟨secret,trace,spc,words,ra,sp,frame⟩ :=
    KeygenSecret.compute keygen hash 0x1204 KeygenSecret.keygen_code s pc level tree side chain seed
      context.levelWord context.leafWord counter context.indexWords context.seedWords
