...
/-- Metadata and seed needed at the start of each signer upper-leaf chain. -/
structure LeafData (s : MachineState) (seed : Seed) (level tree : Nat) (side : Bool) (chain : Nat) : Prop where
...
theorem sign_secret_prepare (hash : Hash) (s : MachineState) (seed : Seed) (level tree : Nat)
    ...
    ∃ final, Trace hash sign s 81 88 1 1 final ∧ final.pc = 0x1698 ∧
      ChainData final level tree side chain 0 (Reference.secret hash seed level tree side chain) ∧
  ...
  obtain ⟨secret, pre, secretPC, secretWords, ra, sp, frame⟩ := KeygenSecret.compute sign hash 0x1584 sign_upper_secret_code
    s pc level tree side chain seed data.levelEq data.leafEq data.chainEq data.indexEq data.seedEq
...
theorem sign_selected_chain (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
  ...
  obtain ⟨ready, pre, readyPC, readyData, readyRA, readySP, readyFrame⟩ :=
    sign_secret_prepare hash s seed level tree side chain pc data
  ...
  obtain ⟨final, steps, cycles, tail, stepsBound, cyclesBound, finalPC, finalData, captured,
    finalRA, finalSP, finalFrame⟩ := sign_chain_captured_endpoint hash ready seed pointer level tree side chain message
...
theorem sign_unselected_chain (hash : Hash) (s : MachineState) (seed : Seed) (pointer level tree : Nat)
    (side : Bool) (chain : Reference.Chain) (pc : s.pc = 0x1584)
  ...
  obtain ⟨final, steps, cycles, tail, stepsBound, cyclesBound, finalPC, finalData,
    finalRA, finalSP, finalFrame⟩ := sign_chain_unselected_endpoint hash ready seed pointer level tree side chain
