...
theorem initializeState_block (s : MachineState) (pc : s.pc = 0x1000) :
  ...
  let s13 := execInstrBr s12 (.ADDI .x10 .x0 2)
  ...
  · have hp : s11.pc = 0x102c := by simp [s1, s2, s3, s4, s5, s6, s7, s8, s9, s10, s11, execInstrBr, pc]
    simp only [fetch, hp]; decide
  · rfl
  apply OrdinarySteps.step s12 s13 _ (.base (.ADDI .x10 .x0 2)) 0
...
theorem initializeState_invariant (s : MachineState) (pc : s.pc = 0x1000) :
    CopyInvariant 0x1034 0x20 0x80020 2 2 (initializeState s) := by
...
theorem messageCopyState_invariant (s : MachineState) (pc : s.pc = 0x104c) :
    CopyInvariant 0x105c 0 0x80030 4 4 (messageCopyState s) := by
...
def randomizerInputWord (s : MachineState) (i : Fin 10) : Word :=
  if i.val = 0 then 6 else if i.val < 4 then 0 else
    ...
    else s.getMem (wordAddress 0 (i.val - 6))
...
theorem entry_randomizer_trace (hash : Hash) (s : MachineState) (pc : s.pc = 0x1000) :
    ∃ ready final, Trace hash sign s 105 120 1 2 final ∧ final.pc = 0x10fc ∧
      (∀ i : Fin 10, ready.getMem (wordAddress 0x80000 i.val) = randomizerInputWord s i) ∧
