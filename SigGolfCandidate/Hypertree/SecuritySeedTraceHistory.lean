import SigGolfCandidate.Hypertree.SecuritySeedTraceBlocks
...
private theorem parsed_seedInputs (history : History) (query : Query) (parsed : Option (Message × Bytes 32))
    (eligible cached : Bool) (answer : BitVec 256) :
    (recordParsed history query parsed eligible cached answer).seedInputs =
      match parsed with
      | some _ => history.seedInputs
      | none => if eligible then query :: history.seedInputs else history.seedInputs := by
  cases parsed with
  | none => cases eligible <;> rfl
  | some pair => cases pair; rfl

/-- Parsed index inputs cannot be seed inputs. Consequently this recorded
history contains exactly the eligible public-query trace, in reverse order. -/
theorem public_seedInputs (pk : PublicKey) (history : History) (query : Query) (cached : Bool) (answer : BitVec 256) :
    (recordPublic pk history query cached answer).seedInputs =
      if SeedEligible query then query :: history.seedInputs else history.seedInputs := by
  rw [recordPublic, parsed_seedInputs]
  cases parsed : SecurityIndexQuery.parse pk query with
  | none => simp only [decide_eq_true_eq]
  | some pair => rw [if_neg (SecurityIndexQuery.parsed_not_seedEligible pk query pair parsed)]
...
def extend {α : Type} (history : History) (result : Option α × List Query) : Option α × List Query :=
  (result.1, history.seedInputs.reverse ++ result.2)

def project {α : Type} (result : SecurityMonitorGraphView.Result α) : Option α × List Query :=
  (result.value, result.history.seedInputs.reverse)
...
theorem extend_public {α : Type} (pk : PublicKey) (history : History) (query : Query)
  ...
  simp only [extend, public_seedInputs, prependPublic]
