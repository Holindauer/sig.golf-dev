...
def seededWith (scheme : Interface) (adversary : Adversary submission.sizes) (rounds : Nat) (seed : Seed) :
    OracleComp World AttackResult := do
  let result ← (scheme.keygen seed).liftComp World
  let some (pk,cache) := result.1 | pure ⟨false,result.2⟩
  interactWith scheme adversary seed pk rounds (adversary.initial pk cache) {hashCalls:=result.2}

theorem experiment_seeded (scheme : Interface) (adversary : Adversary submission.sizes) (rounds : Nat) :
    experimentWith scheme adversary rounds = (do let seed←sampleSeed; observe (seededWith scheme adversary rounds seed) ∅) := by
  change observe (sampleSeed.liftComp World >>= fun seed => seededWith scheme adversary rounds seed) ∅ = _
  exact observe_uniform_bind sampleSeed _ ∅
...
theorem seeded_equivalent_generic (left right : Interface) (adversary : Adversary submission.sizes)
    (rounds : Nat) (seed : Seed) (fact : PublicKey → Hash → Prop)
    (keyEq : ∀ hash, evalWithAnswerFn hash (left.keygen seed)=evalWithAnswerFn hash (right.keygen seed))
    (keyKnown : ∀ result ∈ support (runHash (right.keygen seed) ∅),
    ...
    𝒮[observe (seededWith left adversary rounds seed) ∅]=𝒮[observe (seededWith right adversary rounds seed) ∅] := by
  unfold seededWith
  calc
    _ = 𝒮[observe ((right.keygen seed).liftComp World >>= fun result =>
      match result.1 with
      | some (pk,cache) => interactWith left adversary seed pk rounds (adversary.initial pk cache) {hashCalls:=result.2}
    ...
    _ = _ := by
      ...
      | some pair =>
        ...
        simp only
        exact interact_equivalent_generic left right (fact pk) adversary seed pk rounds
          (adversary.initial pk cache) {hashCalls:=result.1.2} result.2 (keyKnown result mem pk cache value)
          (signEq pk) (checkEq pk)

theorem seeded_equivalent (adversary : Adversary submission.sizes) (rounds : Nat) (seed : Seed) :
    𝒮[observe (seededWith actualInterface adversary rounds seed) ∅]=
      𝒮[observe (seededWith referenceInterface adversary rounds seed) ∅] := by
  apply seeded_equivalent_generic actualInterface referenceInterface adversary rounds seed
    (fun pk hash => Reference.keygen hash seed=pk) (fun hash => keygen_equivalent hash seed)
  · intro result mem pk cache value hash agree
    have eq := (runHash_support_agrees (referenceInterface.keygen seed) ∅ result mem hash agree).2
  ...
  · intro pk hash valid request; exact sign_equivalent hash seed pk request valid
...
theorem experiment_equivalent (adversary : Adversary submission.sizes) (rounds : Nat) :
  ...
  rw [←actual_experiment,experiment_seeded,experiment_seeded]
  apply evalSPMF_bind_congr
  intro seed _
  exact seeded_equivalent adversary rounds seed
