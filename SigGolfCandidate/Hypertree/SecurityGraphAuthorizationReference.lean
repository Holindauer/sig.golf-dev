...
theorem programmed_path_walk (residual : Hash) (seed : Seed) (graph : Labels)
    (level : Fin 160) (index : Nat) (chain : Chain) (point : Fin 8)
    (bound : index < 2 ^ 192) :
    walk (chainHash (programmed (derived residual seed) graph residual)
      level.val (index / 2) (index % 2 == 1) chain) 0 point.val
      (secret (programmed (derived residual seed) graph residual) seed level.val
        (index / 2) (index % 2 == 1) chain) =
      truncate ((factor (derived residual seed, graph)).1 (pathAddress level index chain, point)) := by
  have half : index / 2 < 2 ^ 192 := lt_of_le_of_lt (Nat.div_le_self ..) bound
  have step := programmed_walk residual seed graph (pathAddress level index chain) point.val
...
theorem programmed_canonical_child (residual : Hash) (seed : Seed) (graph : Labels)
    (level : Fin 160) (index : Nat) (bound : index < 2 ^ 192) :
    SecurityPath.canonicalChild (programmed (derived residual seed) graph residual) seed
      level.val index =
      truncate ((factor (derived residual seed, graph)).2.2
        (.node ⟨level.val - 1, by omega⟩ (BitVec.ofNat 192 index))) := by
  have root := programmed_treeRoot residual seed graph
...
theorem programmed_earlier_point (residual : Hash) (seed : Seed) (graph : Labels)
    ...
    (exposure : SecurityPath.EarlierPointExposure
      (programmed (derived residual seed) graph residual) seed level.val index message signature) :
    ∃ chain,
      ¬Authorized (factor (derived residual seed, graph)).2.2 signed
        (pathAddress level index chain, digit message chain) ∧
      signature.values chain = truncate ((factor (derived residual seed, graph)).1
        (pathAddress level index chain, digit message chain)) := by
  apply earlier_point_unauthorized _ seed _ signed level index bound upper message signature
    (programmed_canonical_child residual seed graph level index bound) _ exposure
  intro chain same
  exact same.trans (programmed_path_walk residual seed graph level index chain (digit message chain) bound)

/-- Only indices occurring in actual signing responses open bottom sources. -/
def signedIndices (hash : Hash) (seed : Seed) (history : SecurityForgery.History) :
    Finset (BitVec 160) :=
  (history.map fun entry => SecurityForgery.index hash seed entry.1 entry.2).toFinset

theorem fresh_index_not_signed (hash : Hash) (seed : Seed) (history : SecurityForgery.History)
    (message : Message) (signature : SignatureEncoding.Compact)
    (fresh : ∀ entry ∈ history, SecurityForgery.index hash seed entry.1 entry.2 ≠
      SecurityForgery.index hash seed message signature) :
    SecurityForgery.index hash seed message signature ∉ signedIndices hash seed history := by
...
theorem programmed_new_bottom (residual : Hash) (seed : Seed) (graph : Labels)
    ...
    (exposure : SecurityForgery.NewBottomExposure
      (programmed (derived residual seed) graph residual) seed history message signature) :
    let hash := programmed (derived residual seed) graph residual
    let index := SecurityForgery.index hash seed message signature
    ¬Authorized (factor (derived residual seed, graph)).2.2 (signedIndices hash seed history)
      (pathAddress 0 index.toNat 0, 0) ∧
    signature.bottom = truncate ((factor (derived residual seed, graph)).1
  ...
  refine ⟨bottom_source_unauthorized _ _ _
    (fresh_index_not_signed hash seed history message signature exposure.1), ?_⟩
  exact exposure.2.trans (programmed_path_walk residual seed graph 0 index.toNat 0 0 bound)
