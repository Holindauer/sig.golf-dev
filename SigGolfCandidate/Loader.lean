...
/-- The signer's seed bytes are intact after all public inputs and the stack register are loaded. -/
theorem sign_seed (submission : Submission) (valid : (submission.image .sign).Valid submission.sizes)
    (seed : Seed) (pk : PublicKey) (cache : Cache) (message : Message) (s : MachineState)
    (loaded : initialState submission .sign (seed, pk, cache, message) = some s)
    (i : Nat) (hi : i < 16) :
    s.getByte (BitVec.ofNat 64 (0x20 + i)) = seed.extractLsb' (8 * i) 8 := by
...
/-- info: 'SigGolfCandidate.Loader.sign_seed' depends on axioms: [propext, Classical.choice, Quot.sound] -/
#guard_msgs in
#print axioms sign_seed
