...
from tests.support import ROOT, write_root


class SourceDownloadTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="sig-source-download-")
        ...
        write_root(files, notes="## Original checked idea\n")
        self.sub = SimpleNamespace(id="1" * 32, commit="a" * 40, track="full", detail_dict={"contract": "b" * 64})
        self.meta = source_archive.archives.save_source(source_archive.directory(), self.sub.id, files,
            ...
            submission_root=ROOT, contract="b" * 64)
    ...
    def test_download_is_attachment_and_matches_digest(self):
        ...
        self.assertIn("sig-golf-full-", response.headers["content-disposition"])
