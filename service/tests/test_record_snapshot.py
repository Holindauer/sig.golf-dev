...
from tests.support import EPOCH, ROOT, pin_contract
...
class FakeGit:
    def __init__(self, test):
        ...
        self.source = 'a' * 40
        self.ref = 'refs/tags/sig-source/' + '1' * 32
        self.root = ROOT
        self.files = {'Scheme.lean': b'scheme\n', 'Solution.lean': b'proof\n', 'sigma.txt': b'2274\n',
                      'hverify.txt': b'160\n', 'bound.txt': b'[[4,1,1,0,128]]\n', 'NOTES.md': b'## Idea\n'}
        self.source_tree = self.tree({name: self.blob(raw) for name, raw in self.files.items()})
        ...
        self.original_template = self.blob(b'pull request template')
        self.commits[self.head] = self.tree({'README.md': self.original_readme,
            '.github': self.directory(self.tree({'PULL_REQUEST_TEMPLATE.md': self.original_template})),
...
class RecordSnapshotTests(unittest.TestCase):
    ...
    def submission(self, sigma=2274, hverify=160):
        total = sum(len(raw) for raw in self.git.files.values())
        meta = dict(version=1, sha256='d' * 64, size_bytes=1000, file_count=len(self.git.files), total_bytes=total,
                    source_repo='https://github.com/owner/entries.git', commit=self.git.source,
                    track='full', submission_root=self.git.root, contract=EPOCH)
        ...
        return Submission(id='1' * 32, track='full', status='verified', is_record=True,
                          sigma=sigma, hverify=hverify, score=str(sigma * hverify), commit=self.git.source,
                          source_repo='https://github.com/owner/entries.git',
    ...
    def test_copies_only_checked_root_and_registry_and_preserves_main(self):
        ...
        self.assertEqual(self.git.at('.github/PULL_REQUEST_TEMPLATE.md'), self.git.original_template)
        ...
        self.assertEqual((entry['sigma'], entry['hverify'], entry['score']), (2274, 160, '363840'))
        ...
        self.assertTrue(commit['message'].startswith('Record full: 363840 = 2274 B × 160 (PR #7)'))
    ...
    def test_concurrently_published_newer_record_prevents_retry_rollback(self):
        newer = self.old_entry(id='2' * 32, source_ref='refs/tags/sig-source/' + '2' * 32,
                               sigma=2000, hverify=160, score=str(2000 * 160), finished_at='2026-09-19T13:00:00Z')
    ...
    def test_later_equal_score_and_better_score_never_replace_the_existing_record(self):
        for old_sigma, old_time in [(2274, '2026-09-19T11:00:00Z'), (2000, '2026-09-19T13:00:00Z'),
                                    (2000, '2026-09-19T11:00:00Z')]:
            with self.subTest(sigma=old_sigma, at=old_time):
                old = self.old_entry(id='2' * 32, source_ref='refs/tags/sig-source/' + '2' * 32,
                                     sigma=old_sigma, hverify=160, score=str(old_sigma * 160), finished_at=old_time)
    ...
    def test_better_score_replaces_an_older_worse_entry(self):
        old = self.old_entry(id='2' * 32, source_ref='refs/tags/sig-source/' + '2' * 32,
                             sigma=3000, hverify=160, score=str(3000 * 160), finished_at='2026-09-19T11:00:00Z')
        self.git.set_registry({'version': 1, 'records': {self.sub.track: old}})
        self.assertTrue(record_snapshot.publish_record(self.sub)['published'])
        self.assertEqual(self.git.registry()['records'][self.sub.track]['id'], '1' * 32)
    ...
    def test_unverified_or_unpinned_inputs_fail_before_network(self):
        for changes in [{'status': 'publishing'}, {'is_record': False}, {'score': '1'}, {'sigma': None}]:
    ...
    def test_source_tree_must_contain_the_required_files(self):
        del self.git.trees[self.git.source_tree]['bound.txt']
        detail = self.sub.detail_dict
        detail['source_archive']['file_count'] -= 1
        detail['source_archive']['total_bytes'] -= len(self.git.files['bound.txt'])
        self.sub.detail = json.dumps(detail)
        with self.assertRaisesRegex(record_snapshot.SnapshotError, 'archive layout'):
            record_snapshot.publish_record(self.sub)
        self.assertEqual(self.writes(), [])
    ...
    def test_new_contract_may_replace_an_older_epoch_but_never_a_newer_event(self):
        old = self.old_entry(id='2' * 32, source_ref='refs/tags/sig-source/' + '2' * 32,
                             contract='c' * 64, sigma=1, hverify=1, score='1', finished_at='2026-09-19T11:00:00Z')
