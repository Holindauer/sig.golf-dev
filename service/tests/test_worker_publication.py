...
from tests.support import ROOT, open_admission, pin_contract


class WorkerPublicationTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix='sig-publication-test-')
    ...
    def submission(self, key='a', status='admitting', publication_status=None, sigma=2274, hverify=160):
        sid = key * 32
        receipt = dict(source_ref=f'refs/tags/sig-source/{sid}', created_at='2026-09-19T01:02:03.000004Z',
                       ...
                       assisted_by='A model', contract_commit='f' * 40, submission_root=ROOT)
        ...
        scored = publication_status == 'verified'
        with self.sessions() as session:
            sub = Submission(id=sid, user_id=self.user_id, track='full', status=status,
                             sigma=sigma if scored else None, hverify=hverify if scored else None,
                             score=str(sigma * hverify) if scored else None, commit=key * 40,
    ...
    def finish(self, sub, sigma=2274, hverify=160, status='verified'):
        result = {'status': status, 'track': 'full', 'commit': sub.commit}
        if status == 'verified':
            result.update(sigma=sigma, hverify=hverify, score=str(sigma * hverify))
        with patch.object(worker, 'run_pipeline', return_value=(result, None)) as run:
    ...
    def test_frontier_record_that_is_not_the_best_score_never_updates_main(self):
        best = self.submission(status='verified', publication_status='verified', sigma=2000, hverify=200)
        with self.sessions() as session:
            current = session.get(Submission, best.id)
            current.is_record, current.finished_at = True, utcnow()
            session.commit()
        corner = self.submission('b', status='publishing', publication_status='verified', sigma=1500, hverify=300)
        with patch.object(worker.github, 'post_comment', return_value=42), patch.object(worker.github, 'post_status'):
            worker.deliver_report(corner.id)
        self.publish_snapshot.assert_not_called()
        self.assertTrue(self.load(corner.id).is_record)
        self.assertEqual(self.load(corner.id).status, 'verified')

    def test_delayed_historical_report_cannot_publish_before_unsnapshotted_new_frontier(self):
        older = self.submission(status='verified', publication_status='verified', sigma=2000, hverify=200)
        newer = self.submission('b', status='verified', publication_status='verified', sigma=1900, hverify=200)
        with self.sessions() as session:
            for sub in (older, newer):
                current = session.get(Submission, sub.id)
                current.is_record, current.finished_at = True, utcnow()
                current.detail = json.dumps({k: v for k, v in current.detail_dict.items() if k != 'publication_status'})
            session.commit()
    ...
    def test_detached_new_improvement_is_eligible_while_results_lock_is_held(self):
        older = self.submission(status='verified', publication_status='verified', sigma=2100, hverify=200)
        with self.sessions() as session:
            current = session.get(Submission, older.id)
            current.is_record, current.finished_at = True, utcnow()
            session.commit()
        newer = self.submission('b', status='publishing', publication_status='verified', sigma=2000, hverify=200)
        def publish(checked):
    ...
    def test_source_link_is_exact_and_does_not_require_a_local_zip(self):
        sub = self.submission()
        self.assertEqual(sub.source_url, 'https://github.com/owner/repo/tree/' + sub.commit + '/' + ROOT)
    ...
    def test_finish_order_and_equal_metrics_record_survive_publication_delay(self):
        first = self.submission(status='pending')
        second = self.submission('b', status='pending')
        self.finish(first)
        self.finish(second).assert_not_called()
    ...
    def test_running_proof_retains_result_when_recovery_fails_before_it_finishes(self):
        ...
        def finish_during_failed_recovery(_sub):
            ...
            return {'status': 'verified', 'track': 'full', 'sigma': 2274, 'hverify': 160, 'score': '363840',
                    'commit': sub.commit}, None
