...
from tests.support import ROOT, open_admission, pin_contract, write_root


class RecoveryTests(unittest.TestCase):
    def setUp(self):
        self.temp = tempfile.TemporaryDirectory(prefix="sig-rebuild-test-")
        ...
        for item in [patch.object(settings, "data_dir", self.data),
                     ...
                     patch.object(settings, "bot_login", "sig-bot"),
    ...
    def entry(self, **changes):
        value = dict(id=self.sid, track="full", commit=self.commit, status="pending",
                     contract=contract.contract_id(), source_ref=f"refs/tags/sig-source/{self.sid}",
    ...
    def row(self, *, entry=None, status="verified"):
        ...
        with self.sessions() as session:
            ...
            sub = Submission(id=self.sid, track=entry["track"], commit=self.commit, source_repo="https://github.com/owner/entries.git",
                             ...
                             sigma=2274 if scored else None, hverify=160 if scored else None,
                             score="363840" if scored else None, status=status,
    ...
    def test_closed_force_pushed_receipt_restores_frozen_author_and_pending_head(self):
        ...
        verify.assert_called_with("owner/entries", self.sid, self.commit, f"refs/tags/sig-source/{self.sid}")
    ...
    def test_pending_ref_missing_or_mismatched_is_not_queued(self):
        ...
        with patch("app.resync.github.verify_source_ref") as verify:
            result = self.restore([self.entry(source_ref="refs/tags/sig-source/" + "2" * 32)])
        self.assertEqual(result["restored"], 0)
    ...
    def test_completed_verdict_survives_missing_tag_and_never_fetches_zip_on_sync(self):
        value = self.entry(status="verified", finished_at="2026-09-01T11:00:00Z")
        ...
        with self.sessions() as session:
            sub = session.get(Submission, self.sid)
            self.assertEqual((sub.status, sub.sigma, sub.hverify, sub.score), ("verified", 2274, 160, "363840"))
            self.assertTrue(sub.is_record)
            self.assertIsNone(sub.log_path)

    def test_inconsistent_metrics_in_a_verdict_are_dropped(self):
        value = self.entry(status="verified", finished_at="2026-09-01T11:00:00Z", sigma=2274, hverify=160, score="1")
        self.assertEqual(self.restore([value])["restored"], 0)
    ...
    def test_retry_receipt_uses_event_time_and_copied_old_receipt_cannot_win(self):
        ...
        self.assertEqual(resync.latest_verdicts(comments, "sig-bot")[0][0]["status"], "pending")
        verified = self.entry(status="verified", finished_at="2026-09-01T12:00:00Z")
        ...
        for order in (comments, comments[::-1]):
            self.assertEqual(resync.latest_verdicts(order, "sig-bot")[0][0]["status"], "verified")
    ...
    def make_expected(self, sub):
        ...
        write_root(files, notes="Original idea\n")
    ...
    def test_cache_reconstructs_exact_bytes_and_historical_manifest_from_base_repo(self):
        ...
        with self.sessions() as session:
            ...
            self.assertEqual((current.status, current.score, current.finished_at), (sub.status, sub.score, sub.finished_at))
    ...
    def test_real_bounded_export_preserves_binary_bytes_without_candidate_execution(self):
        ...
        subprocess.run(["git", "-C", str(repo), "-c", "user.name=Fixture", "-c", "user.email=fixture@example.invalid",
                        "-c", "commit.gpgsign=false", "commit", "--quiet", "-m", "source fixture"], check=True, capture_output=True)
        ...
        self.assertEqual((destination / self.root / "sigma.txt").read_text(), "2274\n")
