...
class RetentionRefTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.ref = "refs/tags/sig-source/" + self.sid
    ...
    def test_existing_tag_is_read_only_and_exact(self):
        ...
        self.assertTrue(str(requests[0].url).endswith("/git/ref/tags/sig-source/" + self.sid))
    ...
    def test_recovery_requires_the_expected_tag_and_repository(self):
        for repo, sid, sha, ref in (
                ...
                (self.repo, self.sid, self.sha, "refs/tags/ots-source/" + self.sid)):
    ...
    def test_receipt_metadata_round_trips_without_breaking_html_comment(self):
        entry = dict(id=self.sid, track="full", commit=self.sha, status="pending",
        ...
        self.assertTrue(block.startswith("<!-- sig-result\n"))
        restored = github.parse_verdicts(block)[0]
        for key, value in entry.items():
            self.assertEqual(restored[key], value)

    def test_metrics_in_verdict_blocks_must_be_consistent(self):
        good = dict(track="full", commit=self.sha, status="verified", sigma=2274, hverify=160, score="363840")
        self.assertEqual(len(github.parse_verdicts(github.verdict_block([good]))), 1)
        for bad in (dict(good, score="1"), dict(good, sigma=0), dict(good, hverify="160"), dict(good, score=363840),
                    dict(good, sigma=None)):
            with self.subTest(bad=bad):
                self.assertEqual(github.parse_verdicts(github.verdict_block([bad])), [])
...
class HostedAdmissionTests(unittest.TestCase):
    def setUp(self):
        ...
        tmp = tempfile.TemporaryDirectory(prefix="sig-admission-receipt-")
    ...
    def queue(self, description="Prepared proof"):
        return main.queue_submission(self.session, self.user, "full",
    ...
    def test_worker_cannot_run_until_receipt_publication(self):
        with patch.object(github, "ensure_source_ref", side_effect=lambda repo, sid, sha: "refs/tags/sig-source/" + sid) as pin:
            sub = self.queue()
        ...
        self.assertEqual(sub.detail_dict["receipt"]["submission_root"], "formal/Submissions/Full")
    ...
    def test_retained_retry_does_not_reuse_a_shared_legacy_comment(self):
        with patch.object(github, "ensure_source_ref", side_effect=lambda repo, sid, sha: "refs/tags/sig-source/" + sid):
            sub = self.queue()
            sub.status = "failed"
