...
class ServiceSecurityTests(unittest.TestCase):
    ...
    def test_browser_errors_are_readable_and_api_errors_remain_json(self):
        ...
        with patch('app.main.records.board', side_effect=RuntimeError('private diagnostic')):
    ...
    def test_closed_admission_refuses_pull_requests_with_a_clear_reason(self):
        user = User(login='alice')
        self.session.add(user)
        self.session.commit()
        self.assertEqual(contract.primary_track()['admission'], 'closed')
    ...
    def test_simultaneous_admission_cannot_bypass_per_user_limit(self):
        ...
        def submit(n):
            with sessions() as session:
                ...
                try:
                    main.queue_submission(session, user, 'full', 'local', f'{n:040x}', None, [], None, None, None)
                    return 200
