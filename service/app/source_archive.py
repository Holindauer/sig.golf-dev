...
def download_response(sub) -> Response:
    ...
    filename = f"sig-golf-{meta['track']}-{meta['commit'][:12]}.zip"
...
# `verifier.verify.export_root(source, commit, root, destination, limits)` copies only the flat,
# bounded regular files of `root` at `commit` into `destination` and returns the resolved commit.
_EXPORT_SCRIPT = """
import json, signal, sys
from pathlib import Path
sys.path.insert(0, sys.argv[1])
from verifier.verify import export_root
...
signal.alarm(600)
try:
    limits = json.loads((Path(sys.argv[1]) / "challenges.json").read_text(encoding="utf-8"))["limits"]
    commit = export_root(sys.argv[2], sys.argv[3], sys.argv[4], Path(sys.argv[5]) / sys.argv[4], limits)
    print(commit)
...
def rebuild_cache(sub) -> tuple[dict, bool]:
    ...
    with tempfile.TemporaryDirectory(prefix="sig-source-rebuild-", dir=settings.work_dir) as temporary:
