"""Publish the current best verified root on the submissions default branch via Git Data APIs.
...
def _metrics(value: dict) -> tuple[int, int]:
    sigma, hverify, score = value.get("sigma"), value.get("hverify"), value.get("score")
    limit = contract.max_metric()
    if (type(sigma) is not int or type(hverify) is not int or not 0 < sigma <= limit or not 0 < hverify <= limit
            or not isinstance(score, str) or score != str(sigma * hverify)):
        raise SnapshotError("invalid record metrics")
    return sigma, hverify
...
def _validate_entry(value, slug: str, repo: str) -> dict:
    ...
    if (not isinstance(sid, str) or not source_archive.archives.SUBMISSION_ID.fullmatch(sid)
            ...
            or value.get("source_ref") != f"{github.SOURCE_TAGS}{sid}"
...
def _entry(sub, repo: str) -> dict:
    ...
    if (sub.status != "verified" or not sub.is_record or not sub.current_contract
            ...
            or not sub.scored or not isinstance(sub.finished_at, datetime)):
    ...
    value = dict(id=sub.id, track=sub.track, commit=sub.commit, source_ref=detail.get("source_ref"),
                 ...
                 pr_url=f"https://github.com/{repo}/pull/{sub.pr_number}",
                 sigma=sub.sigma, hverify=sub.hverify, score=sub.score,
...
def _validate_source(git: _Git, entry: dict, required: set[str]) -> None:
    files = git.tree(entry["source_tree"])
    meta = entry["source_archive"]
    if len(files) != meta["file_count"] or not required <= files.keys():
...
def _superseded(old: dict, entry: dict) -> bool:
    """An existing registry entry stays when it is the later event, or the same contract's better score."""
    if _time(old["finished_at"]) >= _time(entry["finished_at"]):
        return True
    if old["contract"] != entry["contract"]:
        return False
    old_sigma, _ = _metrics(old)
    new_sigma, _ = _metrics(entry)
    return not contract.leads(int(entry["score"]), new_sigma, int(old["score"]), old_sigma)


def publish_record(sub) -> dict:
    """Publish the track's best score; caller decides that it is still the track's best.
    ...
    required = contract.required_files(contract.track(sub.track))
    ...
    with httpx.Client(timeout=30, headers=github._headers()) as client:
        ...
        _validate_source(git, entry, required)
        ...
        for attempt in range(MAX_ATTEMPTS):
            ...
            new_commit = _sha(_json(client.post(git.url + "/git/commits", json={
                "message": (f"Record {sub.track}: {sub.score} = {sub.sigma} B × {sub.hverify} (PR #{sub.pr_number})\n\n"
