...
def _metrics(v: dict) -> tuple[int | None, int | None, str | None]:
    if v["status"] != "verified":
        return None, None, None
    sigma, hverify, score = v.get("sigma"), v.get("hverify"), v.get("score")
    if type(sigma) is int and type(hverify) is int and isinstance(score, str) and score == str(sigma * hverify):
        return sigma, hverify, score
    return None, None, None
    ...
    for pr in github.list_pulls(repo):
        ...
        for v, comment_id in history:
            ...
            try:
                ...
                if pinned and v["source_ref"] != f"{github.SOURCE_TAGS}{sid}":
            ...
            with local_lock("results"), SessionLocal() as session:
                ...
                if existing is None:
                    ...
                    pending += int(v["status"] == "pending")
                session.commit()
        if (queue_open_heads and pr.get("state") == "open" and not pr.get("draft", False)) and head not in {
...
def replay_records() -> int:
    ...
    with local_lock("results"), SessionLocal() as session:
        candidates = [s for s in session.scalars(select(Submission).where(
            Submission.status == "verified", Submission.score.is_not(None),
            Submission.finished_at.is_not(None), Submission.pr_number.is_not(None)))
            if contract.track(s.track) is not None and s.scored and s.current_contract and not s.detail_dict.get("demo")
