...
STATUS_CONTEXT = "sig.golf/verifier"
SOURCE_TAGS = "refs/tags/sig-source/"
USER_AGENT = "sig.golf-verifier"
...
def _source_ref_path(owner_repo: str, submission_id: str, commit: str, source_ref: str) -> str:
    if (not REPO_RE.fullmatch(owner_repo) or not settings.submissions_repo
            ...
            or source_ref != f"{SOURCE_TAGS}{submission_id}"):
...
def ensure_source_ref(owner_repo: str, submission_id: str, commit: str) -> str:
    ...
    """
    source_ref = f"{SOURCE_TAGS}{submission_id}"
    url = _source_ref_path(owner_repo, submission_id, commit, source_ref)
...
VERDICT_OPEN, VERDICT_CLOSE = "<!-- sig-result", "-->"
VERDICT_RE = re.compile(r"<!-- sig-result\n(.*)\n-->\s*", re.S)
VERDICT_KEYS = ("id", "track", "commit", "status", "sigma", "hverify", "score", "duration_s", "finished_at",
                "contract", "record", "source_archive", "source_ref", "created_at", "author", "description",
                "co_authors", "assisted_by", "contract_commit", "submission_root", "failure", "git_authors")
...
def _valid_metrics(entry: dict) -> bool:
    sigma, hverify, score = entry.get("sigma"), entry.get("hverify"), entry.get("score")
    if sigma is None and hverify is None and score is None:
        return True
    return (type(sigma) is int and type(hverify) is int and sigma > 0 and hverify > 0
