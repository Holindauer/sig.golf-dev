...
Import `/srv/sig-golf/app/service/deploy/beta.Caddyfile` from the host Caddyfile. It serves all pages, assets, and JSON directly under `https://beta.sig.golf/`, redirects legacy URLs to their root equivalents, and permits fetching presentation metadata from GitHub. The filesystem document root is `/srv/sig-golf/public/site`; links use `/rules.html`, `/submission.html`, `/solver.html`, and `/llms.txt`. Keep the `ots.golf` route unchanged. The optional `OTS_SHARED_VERIFY_LOCK` and `SIG_SHARED_VERIFY_LOCK` must name the same lock file to prevent the two verifiers running together on this host.
